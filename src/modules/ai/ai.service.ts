...
const generateDescription = async (details: { title: string; category?: string; tags?: string[] }) => {
...
const getSmartRecommendations = async (userId: string) => {
    ...
    try {
        // Simple recommendation based on top available courses
...
Return ONLY a JSON object with a 'recommendations' key containing an array of objects with 'id' and 'reason' (max 10 words).`;
        ...
        let aiResults = rawRecs.map((rec: any) => {
        ...
        if (finalResults.length < 4) {
            const filler = upcomingCourses
                ...
                .map(e => ({ ...e, aiReason: "Featured as a trending learning path in SkillBridge." }));
