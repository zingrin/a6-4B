...
const getAllTutors = async (
  ...
  try {
    ...
    if (result.data.length < 1) {
      return res
        .status(200)
        .json({ success: true, message: "No tutors found", data: [] });
    }
...
const getTopTutors = async (
  ...
  try {
    ...
    const result = await tutorService.getTopTutors(limit);

    if (!result.length) {
      return res
        .status(200)
        .json({ success: true, message: "No featured tutors found", data: [] });
    }
...
const deleteTutorSubject = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
