  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      if (paymentData.status === PaymentStatus.COMPLETED) {
        console.log(
          `Payment ${paymentId} already marked as COMPLETED. Skipping.`,
        );
...
const createBookingCheckoutSession = async (bookingId: string, user: User) => {
...
const createCourseCheckoutSession = async (courseId: string, user: User) => {
