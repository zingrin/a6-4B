...
const getOverview = async (instituteId: string) => {
  return await prisma.$transaction(async (tx) => {
    const totalMentors = await tx.mentorProfile.count({
      where: { instituteId },
    });
    ...
    const publishedCount = await tx.course.count({
      where: { instituteId, isPublished: true },
    });
    const draftCount = await tx.course.count({
      where: { instituteId, isPublished: false },
    });
...
const updateMentorProfile = async (
  ...
  if (!mentor)
    throw new Error("Mentor not found or doesn't belong to this institute");
